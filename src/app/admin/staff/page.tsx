...
export default function AdminStaff() {
  ...
  async function handleBulkDelete() {
    ...
    bulk.clear();
    setBulkOpen(false);
    setBulkMsg({ type: "success", text: `Deleted ${count} facilitator${count !== 1 ? "s" : ""}` });
    load();
  }
  ...
  async function handleApproveWithPassword(id: number, temporaryPassword?: string) {
    ...
    const res = await fetch("/api/admin/staff", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    const data = await res.json();
  ...
  async function handleForceActivate(id: number) {
    await fetch("/api/admin/staff", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action: "force-activate" }) });
    load();
  }
        ...
        <Card className="hidden lg:block">
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {items.length === 0 ? (
                  <TableRow><TableCell colSpan={8} className="text-center py-8 text-sm text-slate-400">No facilitators registered yet. Facilitators can self-register, or you can add one.</TableCell></TableRow>
                ) : items.map((s) => (
